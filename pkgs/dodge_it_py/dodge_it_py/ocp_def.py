...
class OCP:
    ...
    def _cost(self, ac_model : AcadosModel) -> AcadosOcpCost:
        ...
        cost_stability = self.h2.stability()
    ...
    def _constraints(self, ac_model : AcadosModel) -> AcadosOcpConstraints:
        ...
        # initial
        q0_vec = np.zeros((self.robot.nq))
        for name,value in self.q0_map.items():
            id = self.robot.model.getJointId(name) - 1 # -1 for missing universe
            q0_vec[id] = value
        ...
        # end
        #   head
        PoS_center = self.h2.PoS_center
        head_pos = self.h2.head_pose

        head_pos_proj = self.proj2PoS(PoS_center, head_pos)
